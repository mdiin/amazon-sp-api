...
class SellingPartner {
  ...
  _decodeBuffer(decrypted_buffer, headers, charset){
    ...
    try {
      return iconv.decode(decrypted_buffer, charset);
    } catch(e){
      throw new CustomError({
        code:'DECODE_ERROR',
        message:e.message
  ...
  async _validateAccessTokenAndRoleCredentials(scope){
    if (this._options.auto_request_tokens){
      ...
      if (!this._role_credentials && this._credentials.aws_user.role){
        // Refresh the role crendentials only if selling partner role is set (temporary vs. long term credentials)
        await this.refreshRoleCredentials();
      }
    }
    if (!this._role_credentials && !this._credentials.aws_user.role){
      this._role_credentials = {
        id:this._credentials.aws_user.id,
        secret:this._credentials.aws_user.secret
      };
    }
  ...
  async refreshRoleCredentials(){
    let signed_request = new Signer(this._region, this._options.use_sandbox, this._options.user_agent, this._api_endpoint).signRoleCredentialsRequest(this._credentials.aws_user);
  ...
  async callAPI(req_params){
    ...
    let signed_request = new Signer(this._region, this._options.use_sandbox, this._options.user_agent, this._api_endpoint).signAPIRequest(token_for_request, this._role_credentials, req_params);
    let res = await request(signed_request);
    ...
    if (json_res.errors){
      ...
      if (res.statusCode === 403){
        ...
        } else if (error.code === 'InvalidInput' && /security token.*invalid/.test(error.message)){
          throw new CustomError({
            code:'SECURITY_TOKEN_INVALID',
            message:this._role_credentials.security_token ?
              'Your security token is invalid. Either provide a valid security token or request a new one by either calling "refreshRoleCredentials()" manually or set "auto_request_tokens" to "true" without passing in role credentials to constructor.' :
              'Please provide valid long term credentials or make sure that you provide AWS_SELLING_PARTNER_ROLE if you are not using long term credentials.'
          });
        }
