...
class Signer {
  ...
  constructor(region, use_sandbox, user_agent, api_endpoint){
    this._region = region;
    this._aws_regions = {
      'eu':'eu-west-1',
      'na':'us-east-1',
    ...
    this._api_endpoint = api_endpoint !== undefined ? api_endpoint : sandbox_prefix + 'sellingpartnerapi-' + this._region + '.amazon.com';
    ...
    if (query){
      ...
      return qs.stringify(query, {
        encoder:(value, defaultEncoder, charset, type) => {
          ...
          return encode_twice ? utils.doubleEncodeURICompenent(value, key_is_array) : utils.encodeURIComponent(value);
        },
        arrayFormat:'comma',
  ...
  signAPIRequest(access_token, role_credentials, req_params){
    ...
    return {
      ...
      headers:{
        ...
        ...(role_credentials.security_token ? {'x-amz-security-token': role_credentials.security_token} : {}), // include only if present (temporary vs. long term)
  ...
  signRoleCredentialsRequest(aws_user){
    ...
    return {
      ...
      headers:{
        ...
        'user-agent': this._user_agent,
        ...(aws_user.session_token ? {'X-Amz-Security-Token': aws_user.session_token} : {}), // include only if available
